...
#define RULE_ID_NONE	       0xFFFF /* процесс не совпал ни с одним правилом */
...
#define _RINGBUF_POW2(x) ((x) <= 1 ? 1 : 1U << (32 - __builtin_clz((unsigned)((x) - 1))))
...
#define RINGBUF_FOPEN_EVENTS \
	512 /* struct file_event=2152B → 512 × 2176 = 1 MB (редкие: unlink/truncate/chmod/chown) */
#endif
...
struct proc_info {
	...
	__u64 file_opens;     /* кумулятивный: сколько раз процесс вызвал openat */
	__u64 socket_creates; /* кумулятивный: сколько раз процесс вызвал socket() */
	...
	__u32 root_pid;	   /* PID корневого процесса дерева отслеживания */
	__u16 rule_id;	   /* индекс правила (RULE_ID_NONE = не совпал) */
	__u8 is_root;	   /* 1 = совпал с правилом напрямую, 0 = потомок */
	__u8 _track_pad;   /* выравнивание */

	/* ── отслеживание вытеснения ─────────────────────────────── */
	__u32 preempted_by_pid; /* tgid последнего вытеснителя (принудит. переключение) */
	char preempted_by_comm
	    [COMM_LEN]; /* comm последнего вытеснителя (резолвлен до главного потока) */
