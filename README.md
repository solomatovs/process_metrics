...
- **25 типов событий**: snapshot, conn_snapshot, fork, exec, exit, oom_kill, file_open, file_close, file_rename, file_unlink, file_truncate, file_chmod, file_chown, net_listen, net_connect, net_accept, net_close, signal, tcp_retrans, syn_recv, rst_sent, rst_recv, udp_agg, icmp_agg, disk_usage
...
| `snapshot_interval` | `3` | Интервал снимков (секунды) |
| `refresh_interval` | `= snapshot_interval` | Интервал обновления /proc (секунды) |
| `exec_rate_limit` | `0` (без лимита) | Лимит exec-событий/сек в ringbuf |
| `log_level` | `1` | 0 = errors, 1 = info, 2 = debug |
| `heartbeat_interval` | `30` | Интервал heartbeat-лога (0 = отключить) |
| `log_snapshot` | `true` | Логировать snapshot строки |
| `log_refresh` | `true` | Логировать refresh строки |
...
| `file_tracking` | open/close/rename/unlink/truncate/chmod/chown + per-fd I/O | `file_open`, `file_close`, `file_rename`, `file_unlink`, `file_truncate`, `file_chmod`, `file_chown` | выкл |
...
`file_tracking` — `enabled`, `track_bytes`, `absolute_paths_only` (отброс относительных путей), `include`/`exclude` (списки префиксов путей), `emit_*` флаги. Отслеживает: open, close (с накопленными read/write/pread/pwrite/readv/writev/sendfile байтами), rename, unlink, truncate, chmod, chown. Кумулятивный счётчик `file_opens` (openat вызовы) и `socket_creates` (socket вызовы) попадают в snapshot.
...
| `file` | 4 МБ | file_open/close/rename/unlink/truncate (~1900 × 2152B) | Массовые файловые мутации |
| `file_ops` | 1 МБ | file_chmod/chown (~480 × 2152B, редкие) | — |
...
|---------|----------|----------|
| `user_name` | uid | Реальный владелец процесса |
| `login_name` | loginuid | Audit UID — реальный пользователь (сохраняется через sudo/su) |
| `session_name` | sessionid (через loginuid) | Имя владельца audit-сессии |
| `euser_name` | euid | Effective UID |

Резолвинг через NSS (`getpwuid_r`) — поддерживает LDAP, SSSD, NIS и локальные /etc/passwd. Результаты кэшируются с rwlock. При неудаче resolve записывается `NOT_FOUND`. `loginuid=4294967295` записывается как `AUDIT_UID_UNSET`, `sessionid=4294967295` — как `AUDIT_SESSION_UNSET`.
...
| `max_connections` | `1` | Макс. одновременных подключений |
| `log_requests` | `true` | Логировать каждый HTTP-запрос |
| `allow` | — | CIDR whitelist (`"10.0.0.0/8"`, `"127.0.0.1"`) |
| `max_buffer_size` | `33554432` (32 МБ) | Размер кольцевого буфера |
...
25 типов событий, разделённых на категории. Подробное описание каждого: [docs/event_types.md](docs/event_types.md).
...
| event_type | Триггер | Ключевые поля | Описание |
|------------|---------|---------------|----------|
| `snapshot` | Каждые `snapshot_interval` секунд | Все 90+ полей (см. [docs/snapshot_fields.md](docs/snapshot_fields.md)) | Полный снимок каждого отслеживаемого процесса. Включает завершившиеся (status=EXITED) процессы однократно. Содержит `file_opens`, `socket_creates` — кумулятивные счётчики syscalls |
| `conn_snapshot` | Вместе с `snapshot` | net_local/remote_addr:port, net_conn_tx/rx_bytes, net_conn_tx/rx_calls, net_duration_ms, state | Снимок каждого живого TCP-соединения (см. [docs/conn_snapshot_fields.md](docs/conn_snapshot_fields.md)). Включает закрытые (CLOSED) соединения однократно. state: `L`=listener, `E`=established |
...
|--------|------|----------|
| Идентификация | pid, ppid, rule_pid, uid, euid, loginuid, sessionid, tty_nr, comm, exec, args | PID, родитель, UID'ы (реальный, эффективный, audit), терминал, командная строка |
...
| Цепочка процессов | process_chain | Цепочка наследования: [pid, ppid, ppid's parent, ..., init] (до 32 уровней). rule_pid всегда в цепочке |
...
(
    ...
    SELECT # removed  AS event_type UNION ALL
...
tests/
  ...
  test_reparent_chain.sh       — тест цепочки process_chain
